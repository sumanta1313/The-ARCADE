...
<main class="shop-main-container grid">
    <div>
      ...
      <%
        String pdcat = request.getParameter("pdcat"); // Get the category from the query string

        String DB_URL = "jdbc:mysql://localhost:3306/user";
        String DB_USER = "root";
        String DB_PASSWORD = "Binit.1234";

        if (pdcat != null && !pdcat.isEmpty()) { // Check if pdcat is provided
            try {
                Class.forName("com.mysql.cj.jdbc.Driver");
                Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
                PreparedStatement pst = connection.prepareStatement("SELECT * FROM product WHERE pdcat = ?");
                pst.setString(1, pdcat); // Bind the category to the query
    ...
    %>
            <p>No category selected or category does not exist.</p>
    <%
        }
    %>
